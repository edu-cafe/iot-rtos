...
void swap(int *pd1, int *pd2, int delay)
{
	temp = *pd1;
	*pd1 = *pd2;
	my_delay(delay);
...
static  void  App_TaskStart (void *p_arg)
{
    ...
    os_err = OSTaskCreate(Task1,"Task1",&Task1_Stk[TASK_STK_SIZE - 1],8);
	sprintf(buff,"\nTask1 Create(err:%d)\n", os_err);
	Puts(buff);
	os_err = OSTaskCreate(Task2,"Task2",&Task2_Stk[TASK_STK_SIZE - 1],9);
	sprintf(buff,"\nTask2 Create(err:%d)\n", os_err);
	Puts(buff);
...
void Task1(void *data)
{
	...
	for (;;) {
		x=10; y=20;
		//sprintf(buff,"%s_before:%d,%d\n", (char *)data, x, y);
		//Puts(buff);
		swap(&x, &y, 1);
		sprintf(buff,"%s_after:%d,%d\n", (char *)data, x, y);
		Puts(buff);
        OSTimeDly(1);                            
	}
}
